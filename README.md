...
セットアップは2通り。どちらも drawzu をクローンして `npm install` 済みであることが前提
（ブリッジは drawzu リポジトリの依存パッケージで動くため）。パスはクローン先の絶対パスに読み替える。

**A. プロジェクト単位** — 使う側プロジェクトに `.mcp.json` を置く:
...
```json
{
  "mcpServers": {
    "drawzu": {
      "command": "npx",
      "args": ["tsx", "/path/to/drawzu/server/mcp.ts"]
    }
  }
}
```

**B. 全プロジェクト共通（ユーザースコープ）** — 1回の登録で、どのプロジェクトの Claude Code からも使える:

```bash
claude mcp add --scope user drawzu -- npx tsx /path/to/drawzu/server/mcp.ts
```

登録先はホームディレクトリの `~/.claude.json`（個人環境の設定で、リポジトリには含まれない）。
A と B が同名で重なっても、プロジェクトスコープ（A）が優先されるだけで害はない。
どのプロジェクトでも図とAIを同じモデルに向けるのが drawzu の目的なので、B を基本とし、
A はリポジトリに設定を同梱したいプロジェクトでのみ使う。

Claude Code を起動し直すと `get_model` / `patch_model` / `to_sql` が使える。
「この要件でテーブル設計してモデルに流し込んで」→ 図に現れる。

**サーバーの手動起動は不要**。ブリッジが接続に失敗したら自分でサーバーを detached 起動する
（`DRAWZU_ROOT` は Claude Code が cwd で spawn するので自然に対象プロジェクトになる）。
ポートはプロジェクトパスのハッシュから決定的に導出（43117〜44116）されるので、
複数プロジェクトで同時に使っても衝突しない。ブラウザで開く URL とサーバーの起動ログは
対象プロジェクトの `.drawzu/server.log` に出る。

**停止も不要**。ブラウザ（WebSocket）が1つも繋がっておらず、最後のアクセスから
30分過ぎたらサーバーは自発終了する。次に必要になればまた自動起動されるので、
体感は「常に生きている」のまま、使っていないプロジェクトのプロセスは残らない。

環境変数（通常は不要）:
- `DRAWZU_URL` … 接続先を明示する（例: 手動起動した特定ポートのサーバーに繋ぐ）
- `DRAWZU_ROOT` … モデルの置き場所を cwd 以外にする
- `DRAWZU_IDLE_TIMEOUT` … アイドル自動停止までの分数（既定 30、`0` で無効）

**⚠️ 手動起動サーバーからの移行**: 同じプロジェクトを `DRAWZU_ROOT=... npm run dev` で
手動起動したサーバーが残っている状態で自動起動に任せると、**2つのサーバーが同じ
`.drawzu/model.json` を書いて競合する**（後勝ちで片方の編集が消える）。自動起動に
切り替える時は先に手動起動分を止めること。ポートが変わるのでブラウザ URL も変わるが、
モデル（テーブル定義・配置）は model.json 経由でそのまま引き継がれる。
どうしても手動起動サーバーを使い続けたい場合は `DRAWZU_URL` でそのポートを明示する。
...
### どのチャットからでも図を開く（/drawzu コマンド）

Claude Code のユーザーレベルコマンドとして `~/.claude/commands/drawzu.md` を置くと、
どのプロジェクトのチャットでも `/drawzu` の一言で「サーバーを起こす → ブラウザでER図を開く →
モデル概要を報告」まで走る。ユーザースコープ登録（前節 B）とセットで使う想定。

コマンド定義（コピーして `~/.claude/commands/drawzu.md` に保存する。パスは読み替える）:

````markdown
---
description: このプロジェクトのDBモデル(ER図)をdrawzuでブラウザに開く
---

このプロジェクトの drawzu ER図ビューをブラウザで開いてください。手順:

1. drawzu の MCP ツール `get_model` を1回呼ぶ（サーバーが立っていなければ、この呼び出しでブリッジが自動起動する）
2. ブラウザで開く URL を特定する:
   - まずカレントプロジェクトの `.drawzu/server.log` の末尾にある `[drawzu] web:   http://localhost:XXXXX` 行から取る
   - ログが無い場合は、次のワンライナーでポートを導出する（MCPブリッジと同じ FNV-1a ハッシュ）:
     ```
     node -e 'let r=process.cwd(),h=0x811c9dc5;for(const c of r){h^=c.codePointAt(0);h=Math.imul(h,0x01000193)}console.log("http://localhost:"+(43117+((h>>>0)%1000)))'
     ```
3. `curl -s <URL>/api/health` で `{"ok":true,...}` が返ることを確認してから、`open <URL>` でブラウザを開く
4. ユーザーに URL と、モデルの現状の要約（テーブル数・テーブル名の一覧程度）を報告する

補足:
- モデルの実体はカレントプロジェクトの `.drawzu/model.json`（無ければ空モデルで自動作成される）
- サーバーはアイドル30分（ブラウザを閉じた状態）で自動停止するが、次回もこのコマンドや `get_model` 呼び出しで勝手に復活する
- `get_model` ツールが見えない場合は、ユーザースコープのMCP登録が外れている。`claude mcp add --scope user drawzu -- npx tsx /path/to/drawzu/server/mcp.ts` を案内する
````

コマンド定義・MCP登録ともユーザーのホーム側（`~/.claude/` / `~/.claude.json`）に置く個人設定で、
このリポジトリには含めない。リポジトリ直下の `CLAUDE.md` / `.claude/` も個人の作業環境設定のため
`.gitignore` で除外している（コミットされている `.mcp.json` は、drawzu リポジトリ自身の開発で
自分のMCPを使うためのもの）。
